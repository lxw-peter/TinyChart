 ...
 *
 * @param {Object} param
 * @param {Array} param.domain  值域
 * @param {Array} param.range   定义域
 * @param {number} param.padding  控制条之间的间距
 * @returns
 */
export function band({ domain, range, padding }) {
  ...
  return { step, bandWidth, bandRange: new Array(n).fill(0).map(x) };
}

/**
 * 输入到起始值的差值与值域两端差值的比例
 * @param {number} value
 * @param {number} start 值域左端值
 * @param {number} stop 值域右端值
 * @returns
 */
